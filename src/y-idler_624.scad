...
module y_idler_cuts(){
	...
	translate([width/2,depth-7,7.7]) rotate([0,90,0]) cylinder(d = 3.3, h = width+1, $fn = 30,center=true);
	translate([width,depth-7,7.7]) rotate([0,90,0])cylinder(d=6.5,h=wall+0.6,$fn=32,center=true);
	...
	// Otvor na matku
	translate([-0.1,depth-7,7.7]) rotate([0,90,0]) cylinder(h = 3.5, d=6.7, $fn=6);	
	// Setreni plastem
	...
	// Lozisko
	%translate([wall+1.5,depth-7,7.7]) rotate([0,90,0]) cylinder(r = 6.5, h = bearing_width, $fn = 30);
}
